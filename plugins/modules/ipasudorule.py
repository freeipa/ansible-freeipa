...
from ansible.module_utils.ansible_freeipa_module import \
    ...
    convert_param_value_to_lowercase
...
def gen_args(entry):
    ...
    if entry.usercat is not None:
        _args['usercategory'] = entry.usercat
    if entry.hostcat is not None:
        _args['hostcategory'] = entry.hostcat
    if entry.cmdcat is not None:
        _args['cmdcategory'] = entry.cmdcat
    if entry.runasusercat is not None:
        _args['ipasudorunasusercategory'] = entry.runasusercat
    if entry.runasgroupcat is not None:
        _args['ipasudorunasgroupcategory'] = entry.runasgroupcat
...
class SudoRuleEntry:
    """Used to create entry objects."""

    @staticmethod
    def identity(value):
        return value

    # Conversion functions to apply to each parameter
    params = {
        "name": (identity,),
        "description": (identity,),
        "cmdcategory": (identity,),
        "usercategory": (identity,),
        "hostcategory": (identity,),
        "runasusercategory": (identity,),
        "runasgroupcategory": (identity,),
        "host": (convert_param_value_to_lowercase,),
        "hostgroup": (convert_param_value_to_lowercase,),
        "hostmask": (identity,),
        "user": (convert_param_value_to_lowercase,),
        "group": (convert_param_value_to_lowercase,),
        "allow_sudocmd": (identity,),
        "allow_sudocmdgroup": (convert_param_value_to_lowercase,),
        "deny_sudocmd": (identity,),
        "deny_sudocmdgroup": (convert_param_value_to_lowercase,),
        "sudooption": (identity,),
        "order": (identity,),
        "runasuser": (convert_param_value_to_lowercase,),
        "runasuser_group": (convert_param_value_to_lowercase,),
        "runasgroup": (convert_param_value_to_lowercase,),
        "nomembers": (identity,)
    }
    ...
    aliases = {
        ...
        "runasusercat": "runasusercategory",
        "runasgroupcat": "runasgroupcategory",
        "option": "sudooption",
        "sudoorder": "order",
    }

    def __getattribute__(self, name):
        name = SudoRuleEntry.aliases.get(name, name)
        return super().__getattribute__(name)


def _extract_entry(module, data, invalid_params, param_get):
    entry = SudoRuleEntry()
    for (param, conversions) in SudoRuleEntry.params.items():
        value = param_get(data, param)
        for fn in conversions:
            value = fn(value)
        setattr(entry, param, value)
    state = module.params_get("state")
    action = module.params_get("action")
    validate_params(module, entry, state, action, invalid_params)
    return entry


def get_entry(module, sudorule_data, invalid_params):
    return _extract_entry(
        module, sudorule_data, invalid_params, dict.get)


def get_entry_simple_case(module, name, invalid_params):
    entry = _extract_entry(
        module, module, invalid_params, IPAAnsibleModule.params_get)
    entry.name = name  # pylint: disable=attribute-defined-outside-init
    return entry


def validate_params(module, entry, state, action, invalid_params):
    if action is None:
    ...
    for param in invalid_params:
        if getattr(entry, param, None) is not None:
            module.fail_json(msg=msg.format(param, state, action))

    if not entry.name:
        module.fail_json(msg="Sudorule 'name' not set.")

    _ensure_entry_is_valid(module, entry, state, action)


def _ensure_entry_is_valid(module, entry, state, action):
    ...
    if entry.hostmask:
        entry.hostmask = [
            to_text(netaddr.IPNetwork(x).cidr) for x in entry.hostmask
        ]

    if state == "present" and action == "sudorule":
        _ensure_entry_is_valid_for_present_sudorule(module, entry)


def _ensure_entry_is_valid_for_present_sudorule(module, entry):
...
def main():
    ...
    if not names and not sudorules:
        ansible_module.fail_json(msg="One of name or sudorules is required")
    ...
    with ansible_module.ipa_connect():
        ...
        for name in names:
            if isinstance(name, dict):
                entry = get_entry(ansible_module, name, invalid)
            else:
                entry = get_entry_simple_case(ansible_module, name, invalid)
