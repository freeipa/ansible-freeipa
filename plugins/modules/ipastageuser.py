...
def find_user(module, name, preserved=False):
    ...
    _result = api_command(module, "stageuser_find", name, _args)
    ...
    try:
            ...
            if state == "present":
                ...
                if action == "user":
                    ...
                    if res_find is not None:
                        ...
                        if not compare_args_ipa(ansible_module, args,
                                                res_find):
                            commands.append([name, "stageuser_mod", args])
                        ...
                        commands.append([name, "stageuser_add", args])
                    ...
                    if len(manager_add) > 0:
                        commands.append([name, "stageuser_add_manager",
                                         {
                                             "user": manager_add,
                                         }])
                    # Remove managers
                    if len(manager_del) > 0:
                        commands.append([name, "stageuser_remove_manager",
                                         {
                                             "user": manager_del,
                                         }])
                    ...
                    if len(principal_add) > 0:
                        for _principal in principal_add:
                            commands.append([name, "stageuser_add_principal",
                                             {
                                                 "krbprincipalname":
                                                 _principal,
                                             }])
                    ...
                    if len(principal_del) > 0:
                        for _principal in principal_del:
                            commands.append([name, "stageuser_remove_principal",
                                             {
                                                 "krbprincipalname":
                                                 _principal,
                                             }])
                    ...
                    if len(certificate_add) > 0:
                        for _certificate in certificate_add:
                            commands.append([name, "stageuser_add_cert",
                                             {
                                                 "usercertificate":
                                                 _certificate,
                                             }])
                    ...
                    if len(certmapdata_add) > 0:
                        for _data in certmapdata_add:
                            commands.append([name, "stageuser_add_certmapdata",
                ...
                if action == "user":
                    if res_find is not None:
                        ...
                        commands.append([name, "stageuser_del", args])
                elif action == "member":
                    if res_find is None:
                        ansible_module.fail_json(
                            msg="No user '%s'" % name)
                    ...
                    if certificate is not None and len(certificate) > 0:
                        for _certificate in certificate:
                            commands.append([name, "stageuser_remove_cert",
                                             {
                                                 "usercertificate":
                                                 _certificate,
                                             }])
                    ...
                    if certmapdata is not None and len(certmapdata) > 0:
                        ...
                        # one reliably (https://pagure.io/freeipa/issue/8097)
                        for _data in certmapdata:
                            commands.append([name, "stageuser_remove_certmapdata",
                                             gen_certmapdata_args(_data)])
            ...
            #        raise ValueError("No preserved user '%s'" % name)
                ...
                if res_find is not None:
                    if res_find["nsaccountlock"]:
                        commands.append([name, "stageuser_activate", {}])
                else:
                    raise ValueError("No staged user '%s'" % name)
            else:
        ...
        for name, command, args in commands:
            try:
                ...
                if "random" in args and command in ["stageuser_add", "stageuser_mod"] \
