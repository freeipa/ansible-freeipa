...
`ipareplica_dot_forwarders` | List of DNS over TLS forwarders (list of strings) | no
`ipareplica_dns_over_tls` \| `ipaclient_dns_over_tls` | Configure DNS over TLS (bool, default: false) | no
...
`ipareplica_dns_over_tls_key` | Key for certificate specified in --dns-over-tls-cert. (string) | no
`ipareplica_dns_policy ` | Encrypted DNS policy. (choice: relaxed, enforced, default: relaxed) | no
