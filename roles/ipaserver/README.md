...
`ipaserver_dot_forwarders` | List of DNS over TLS forwarders (list of strings) | no
`ipaserver_dns_over_tls` \| `ipaclient_dns_over_tls` | Configure DNS over TLS (bool, default: false) | no
...
`ipaserver_dns_over_tls_key` | Key for certificate specified in --dns-over-tls-cert. (string) | no
`ipaserver_dns_policy` | Encrypted DNS policy. (choice: relaxed, enforced, default: relaxed) | no
