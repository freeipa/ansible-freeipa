...
if 'ansible.executor' in sys.modules:
    ...
    if NUM_VERSION >= 40400:
        ...
        [global-dns-domain-*]
        servers={servers}
        """) if not hasattr(tasks, "NM_IPA_CONF") else tasks.NM_IPA_CONF
        ...
        """) if not hasattr(tasks, "RESOLVE1_IPA_CONF") else \
            tasks.RESOLVE1_IPA_CONF
...
def configure_dns_resolver(nameservers, searchdomains, fstore=None):
    ...
    if dnsforwarders is not None:
        resolve1_enabled = dnsforwarders.detect_resolve1_resolv_conf()
    else:
        resolve1_enabled = False
    nm_service = knownservices['NetworkManager']

    # At first configure systemd-resolved
    if resolve1_enabled:
        if not os.path.exists(paths.SYSTEMD_RESOLVED_IPA_CONF):
            ...
            with open(paths.SYSTEMD_RESOLVED_IPA_CONF, "w") as outf:
    ...
    if nm_service.is_enabled():
        if not os.path.exists(paths.NETWORK_MANAGER_IPA_CONF):
            ...
            with open(paths.NETWORK_MANAGER_IPA_CONF, 'w') as outf:
                os.fchmod(outf.fileno(), 0o644)
                outf.write(cfg)
